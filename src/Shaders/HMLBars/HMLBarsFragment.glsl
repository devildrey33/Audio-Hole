...
void main() {
    vec3 color = vec3(0.0);
    color += Bars(vec3(1.0, 0.5, 0.5), 1.6, 1.0);
    color += Bars(vec3(0.5, 1.0, 0.5), 1.3, 0.9);
    color += Bars(vec3(0.5, 0.5, 1.0), 1.0, 0.8);
    color -= Bars(vec3(0.7, 0.7, 0.0), 1.9, .50);
    if (color.r > 0.0 || color.g > 0.0 || color.b > 0.0) {
//        gl_FragColor = vec4(color, 1.0);        
        gl_FragColor = vec4(color, .5 * (1.0 - vUv.x));        
    }
    else {
        discard;
    }
